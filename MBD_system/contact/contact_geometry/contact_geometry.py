...
class ContactGeometry(object):
    ...
    classdocs
    """
    def __init__(self, _type, _area=None, body=None, filename=None, uP1=None, uP2=None, parent=None):
        """
        Constructor
        ...
        if _area is not None and body is not None and filename is None:
            #   area of contact in body CAD CS
            [x_min, x_max, y_min, y_max] = _area

            #   position of contact (plane) in z dimension
            z_dim = self._parent.z_dim
            self.z_dim_lcs = gcs2lcs_z_axis(body.R[2], z_dim)
    ...
    def create_VBO(self):
        ...
        if not self.VBO_created:
            ...
            self.vbo_data = np.hstack((self.contact_nodes, np.tile(np.append(self.body.color_GL, 1), (len(self.contact_nodes), 1))))
