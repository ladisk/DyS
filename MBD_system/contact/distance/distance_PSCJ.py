    ...
    def _evaluate_distance(self):
        ...
        for i in range(0, 2):
            ...
            if (np.pi / 2. < self.phi_list[0] < 3. * np.pi / 2.) and (np.pi / 2. < self.phi_list[1] < 3. * np.pi / 2.):
                ...
                # print "np.dot(self._distance_vector[i], self.n_list[i]) =", np.dot(self._distance_vector[i], self.n_list[i])
                # print "self._distance[i] =", self._distance[i]
...
if __name__ == "__main__":
    ...
    r_iP = np.array([-0.00084160566, -0.000213709577])
    r_iR = np.array([7.675913164978e-05, 2.940790189987e-05])
    r_jP = np.array([9.999999747379e-06, -3.999999898952e-05])
    # r_jP = np.array([9.999999747379e-06, -1.999999898952e-04])
