...
class ContactPointPSCJ(DistancePSCJ):
    ...
    def _create_forces(self, direction=""):
        ...
        if hasattr(self._parent, "body_id_list"):
            ...
            for body_id, visibility in zip(self.body_id_list, self._parent._visible_F_list): #  corrected from self._parent.body_id_list, self._parent._visible_F_list to self.body_id_list, self._parent._visible_F_list
                ...
                #     print "ACTOR ADDED TO RENDERED"
                #     F.renderer = self._parent._parent._parent._parent.dys.simulation_control_widget.vtkWidget.renderer
                #     F.renderer.AddActor(F.vtk_actor)
    ...
    def contact_geometry_LCS(self):
        ...
        # print "self.phi_list =", np.rad2deg(self.phi_list)
    ...
    def contact_points_LCS(self, q):
        ...
        for i, (body_id, rP) in enumerate(zip(self.body_id_list, self.r_P_GCS_list)):
            ...
            self._Fn_list[i]._update_u_P(uP)
            self._Fn_list[i]._update_r_P(rP)

            self._Ft_list[i]._update_u_P(uP)
            self._Ft_list[i]._update_r_P(rP)
    ...
    def contact_points_GCS(self, frame_nodes_GCS=np.zeros([4, 2])):
        ...
        if self.pin_in_section in ["iP", "iR"]:
            rPi = self.r_jPiL_list[["iP", "iR"].index(self.pin_in_section)]
            rPj = self.r_jP
    ...
    def contact_velocity(self, q):
        ...
        :return:
        """
        # print "n_GCS@contact_velocity =", self._n_GCS_list[0], "index =", self.index
