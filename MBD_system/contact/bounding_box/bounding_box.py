...
#         self._boundary(_nodes=self.nodes_LCS, _max=self.max_LCS, _min=self.min_LCS)
    ...
    def _subdivide(self, nodes, normals):
        ...
        for sub_data_obj in self.sub_data_list:
            sub_AABB = AABB(nodes_LCS=sub_data_obj._nodes, normals_LCS=sub_data_obj._normals, min_LCS=sub_data_obj._min, max_LCS=sub_data_obj._max, id_sub=sub_data_obj._id, parent=self, _type = sub_data_obj._type)
...
#                if self._parent._typeInfo == self._typeInfo:
    ...
    def _update_color_GL(self, color):
        """

        :return:
        """
        #   color array of each vertex
        _color = color.astype("float32")
