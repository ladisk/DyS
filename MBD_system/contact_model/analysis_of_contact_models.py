    ...
    def __init__(self, abs_folder_path, file_extension):
        ...
        #     for i, _file in enumerate(files):
            ...
            if _file_extension == file_extension:
                ...
                _sol_data = SolutionData(filename)
                _sol_data.read_file(_file)
...
class Contact_solution_analysis(object):
    ...
    def plot(self, x, y, colors=None):
        ...
        for i, (sol, color) in enumerate(zip(self._solutions, colors)):
            if sol.contact_model._type == "Kelvin-Voigt":
                pass
...
if __name__ == "__main__":
    ...
    # solutions = MBD_solution_analysis(_folder_abs_path, ".xlsx")
    _colors = [[0, 0, 0],
               [.7, .3, .1],
               [1, 0, 0],
               [0, 1, 0],
    ...
    # plt.xlabel(r'\ $\delta \left[m\right]$', fontsize=12)
    plt.ylabel(r'\ $f_n \left[\frac{N}{m}\right]$', fontsize=12)
    # plt.ylabel(r'\ $dR$ [m/s]', fontsize=12)
