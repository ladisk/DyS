...
class JointFixed(Joint):
    ...
    def __init__(self, joint_type, body_id_i, body_id_j, u_iP_CAD=np.array([0, 0]), u_jP_CAD=np.array([0, 0]), u_iQ=np.array([0, 0]), properties_dict={}, parent=None):
        ...
        super(JointFixed, self).__init__(self.joint_type, body_id_i, body_id_j, u_iP_CAD=u_iP_CAD, u_jP_CAD=u_jP_CAD, properties_dict=properties_dict, parent=parent)

        #   auto generated name by type and consecutive number
        self.fixed_joint_id = (sum(1 for joint in self.joints_list if joint.joint_type == "fixed") - 1) + 1
    ...
    def evaluate_C(self, q, t=0.):
        ...
        if self.body_id_i != "ground" or self.body_id_j != "ground":
            ...
            self.C[2] = q2theta_i(q, self.body_id_i) - q2theta_i(q, self.body_id_j)
        elif self.body_id_i == "ground":
            self.C[0:2] = Ai_ui_P_vector(self.u_jP, q2theta_i(q, self.body_id_j))
            self.C[2] = q2theta_i(q, self.body_id_k)
        elif self.body_id_j == "ground":
            self.C[0:2] = Ai_ui_P_vector(self.u_iP, q2theta_i(q, self.body_id_i))
            self.C[2] = q2theta_i(q, self.body_id_i)
        else:
