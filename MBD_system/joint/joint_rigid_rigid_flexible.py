...
class JointRigidRigidFlexible(Joint):
    """
    ...
    def _evaluate_C_q_j(self, q):
        ...
        #   constrain of rotation (gradients)
        if self.n_CNC == 3:
            # C_q_j[2, self.node_id_j * node_e_n:self.node_id_j * node_e_n + 2] = np.ones(2)
            # print "test =", Ai_ui_P_vector(self.u_iP_LCS, q2theta_i(q, self.body_id_i))
            # C_q_j[2, self.node_id_j * node_e_n + 2:self.node_id_j * node_e_n + 4] = Ai_theta_ui_P_vector(self.u_iP_LCS, q2theta_i(q, self.body_id_i), 0)
            # print "AiuiR =", Ai_ui_P_vector(self.u_iP_LCS, q2theta_i(q, self.body_id_i))
            C_q_j[2, self.node_id_j * node_e_n + 2:self.node_id_j * node_e_n + 4] = Ai_ui_P_vector(self.u_iR, q2theta_i(q, self.body_id_i))

        elif self.n_CNC == 4:
            if C_q_j[2:4, self.node_id_j * node_e_n + 2:self.node_id_j * node_e_n + 4].shape == np.eye(2).shape:
                C_q_j[2:4, self.node_id_j * node_e_n + 2:self.node_id_j * node_e_n + 4] = -np.eye(2)

            else:
                raise ValueError, "Submatrix size not equal to size of matrix to be inserted!"

        else:
            print ValueError, "Joint attribute n_CNC not correct!"
